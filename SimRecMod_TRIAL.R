   ...
   ## 1.4 Group priors
    ...
    for(i in 1:ngroups){
      for(k in 2:nyears){
        ...
        G[i,k] ~ dnorm(g.mu[i,k], 1 / (g.mu[i,k] + 0.00001))T(0,)
      }#k
    }#i
...
params <- c("P","var.group", "G.mean", "gamma.mean", "n.est", "annual.s",
            "phi", "colo", "psi", "p11", "p10", "b", "b0.surv") 
...
# Call JAGS 
out <- jags(win.data, inits, params, "IPMmodel_TRIAL.txt", n.chains=nc, n.thin=nt, n.iter=ni, 
...
betas <- data.frame("colo.alpha" = colo.alpha,
                    ...
                    "phi.alpha" = phi.alpha,
                    "phi.beta" = phi.beta,
                    "b0.surv" = out$BUGSoutput$mean$b0.surv[1:14],
                    "surv.sd" = out$BUGSoutput$sd$b0.surv[1:14])

datums <-list(betas, n.est, G.mean2, s2, P2, gamma2)
save(datums, file ="C:/Users/allison/Documents/Project/Dissertation/Recruitment/Results/SimulationResults/TRIALout1_full.RData")
...
cat("
    ...
    for(k in 1:(nyears-1)){
      ...
      eps.surv[k] ~ dnorm(betas[k,5], 1 / (betas[k,6] * betas[k,6]))
    ...
    for(k in 2:nyears){
      ...
      N.ps[k] ~ dpois((P[k-1] + sum(colo[,k-1]*(area[,1]/T - P[k-1])) - P[k-1] * (1 - sum(phi[,k-1]))) * G.mean[k-1])
...
params2 <- c("P", "annual.s", "N.tot", "gamma.mean", "G.mean", 
             "N.rec", "base.s", "hazard", "colo", "phi", "eps.surv") 
