...
#             This model is using group count data and occupancy data from MT
#             to estiamte recruitment with survival fixed at a specific rate
#             from years 2007-2014 because that is when I have occupancy data for
...
# encounter data can't contain 0s for the model I use
...
# Set my constants: em.group is a 10% dispersal rate from a pack, survival is set to 
# 0.75, territory size is set to 600 sq km, and territory overlap is set to the 
# values determined by FWP
em.group <- 0.1
s.prob <- 0.75
Tr <- 600
T.overlap <- c(1.17, 1.11, 1.13, 1.16, 1.24, 1.25, 1.31, 1.24)
...
cat("
    ...
    for(i in 1:ngroups){
      for(k in 2:nyears){
        g.mu[i,k] <- G[i,k-1] * s[k-1] * (1 - em.group) + gamma[i,k-1] # + sigma.proc.group[k-1])
        G[i,k] ~ dnorm(g.mu[i,k], 1/(g.mu[i,k]+.01))T(0,100)
      }
    }
    ...
    for(i in 1:ngroups){
      for(k in 1:nyears){
        y.group[i,k] ~ dnorm(G[i,k], tauy.group)T(0,100)
      }
    }
...
# Data
win.data <- list("nsites"=nsites, "nyears"=nyears, "ngroups"=ngroups, "area"=sitecovs$AREASAMP,
                 ...
                 "acv"=array(unlist(survcovs$ACV), dim=c(695, 5, 8)), "T"=Tr, "s.prob"=s.prob, 
                 "em.group"=em.group, "y.group"=y.group, "T.overlap"=T.overlap)
